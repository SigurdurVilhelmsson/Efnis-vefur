...
- 2026-09-26 — Siggi's answers before Phase 4 (full options: conversation of this date, summarised here):
  hosting on **Cloudflare Workers with static assets**, not Pages (Cloudflare's Pages docs: "Start new projects with Workers"; `sveltia-cms-auth` is a Worker anyway, so one product to learn). Implies a `wrangler.jsonc` and `wrangler` as a pinned dev dependency.
  Repo **`Efnafraedifelag-Islands/Efnis-vefur`**, to be made **private** (GitHub disables scheduled workflows in public repos after 60 days without activity, which would silently stop the daily rebuild) and given a second owner — both manual steps for Siggi; it is public with one owner for now.
  Editors: own GitHub accounts, OAuth via `sveltia-cms-auth`, **direct publishing** (no editorial workflow). Demo on the default `*.workers.dev` address with `noindex` (removed at go-live).
  Defaults accepted: daily rebuild 00:15 UTC via GitHub Action + deploy hook secret; `redirects-draft.txt` becomes `public/_redirects` for the demo.
- 2026-09-26 — Content answers: orphaned WordPress pages become subpages of the two hubs, keeping WP slugs (Landskeppni, Ólympíulið, Bókagjafir under `/menntun/`; Orðaskrá, Sprengjugengið under `/efnafraedi-a-islandi/`); "Efnafræðinám og keppnir" redirects to `/menntun/`. Tenglar at `/tenglar/` (footer). Board history shown verbatim below the current board (the mislabelled "2025–2026" previous board needs checking). 2024 conference gets `2024.en.md` from WordPress; bilingual news item 2024-09-08 split into `.is.md`/`.en.md`; English-only item 2024-10-25 gets an Icelandic version from Siggi. "Icelandic Chemistry Society" → "Chemical" where it is the society's name. Real upcoming events + a date-free fallback. Facebook group link in the footer and on the About page.
- 2026-09-26 — Events: a start time of 00:00 means "time not decided" and shows the date only (CMS hint says so; hint wording is Claude's, awaiting Siggi's approval). Events count as upcoming until the end of their day (Iceland = UTC). Placeholder events removed; first real event is the joint conference with Fagfélag matvælafræðinga, 2 April 2027.
- 2026-09-26 — Board years: the current board is **2026–2027** (the WordPress page mislabels it 2025–2026); Benjamín's board in "Fyrri stjórnir" is correctly 2025–2026. Roles for four members stay TODO until Siggi fills them in.
- 2026-09-26 — 2027 is the **11th** Efnís conference (joint with Fagfélag matvælafræðinga), Icelandic + English pages. Conferences got optional `registration_url`/`registration_deadline`/`abstract_url`/`abstract_deadline`/`abstract_template`; the site only **links** to external forms (Siggi plans Tally + Airtable on the society's accounts) — no embeds, so public pages still make no third-party requests. After a deadline the button becomes a "closed" note (open through the deadline day, via the daily rebuild). New Icelandic UI strings marked `// (UI)` in `src/lib/i18n.ts` await Siggi's approval.
- 2026-09-26 — Next-event fallback is now date-free ("Enginn viðburður er á dagskrá í augnablikinu…"), so it can't go stale.
